...
jobs="$(
  curl -sS -A "$UA" -H "Cookie: __Secure-next-auth.session-token=$SESSION_TOKEN" \
       ...
       -- "https://www.midjourney.com/api/app/recent-jobs/?orderBy=new&jobStatus=completed&userId=${USER_ID}&dedupe=true&refreshApi=0&amount=50&page=$PAGE&jobType=upscale"
)"

# https://www.midjourney.com/api/app/recent-jobs/?amount=50&dedupe=true&isPublished=true&jobStatus=completed&jobType=upscale&minRankingScore=0&orderBy=hot&page=9&refreshApi=0&search=advanced&service=null&userId=8279a880-e345-48b1-a207-c77385215a29&user_id_ranked_score=0%2C3%2C4%2C5


mkdir -p jobs
echo "$jobs" | jq > jobs/last.json
